...
CONSTRAINT R_86 FOREIGN KEY (Cust_Id) REFERENCES T13_CUSTOMER (Cust_Id) ON DELETE CASCADE ON UPDATE CASCADE
);
CREATE UNIQUE INDEX XPKINCIDENT_REPORT_18 ON T13_INCIDENT_REPORT (Incident_Report_Id ASC,Incident_Id ASC,Cust_Id ASC);
...
CONSTRAINT R_85 FOREIGN KEY (Cust_Id) REFERENCES T13_CUSTOMER
(Cust_Id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
REFERENCES T13_PREMIUM_PAYMENT (Premium_Payment_Id, Cust_Id) ON DELETE CASCADE ON UPDATE CASCADE
);
CREATE UNIQUE INDEX XPKRECEIPT_21 ON T13_RECEIPT (Receipt_Id ASC,Premium_Payment_Id ASC,Cust_Id ASC);
